...
export default function PsmHome() {
  ...
  const { getItem, loading } = useEvaluacionPeriodica();
  ...
  React.useEffect(() => {
    ...
    if (hcId) {
      getItem(parseInt(hcId)).then((data) => {
        // Si tu API retorna un array:
        setEvaluaciones(Array.isArray(data) ? data : data ? [data] : []);
      });
    }
  }, []);
 ...
 <button
    className="flex items-center gap-2 rounded-lg bg-red-500/10 px-4 py-2 text-red-700 font-semibold shadow-sm hover:bg-red-500/20 transition-colors mr-4"
    title="Finalizar y salir de la historia clínica"
    onClick={() => {
                    ...
                    <div key={e.id} className="flex items-center justify-between px-4 py-3 hover:bg-white dark:hover:bg-gray-800 transition-colors">
                      <div>
                        <span className="font-medium text-gray-900 dark:text-white">
                          {e.fechaEvalucacion || "Sin fecha"}
                        </span>
                        <span className="text-gray-600 dark:text-gray-400 ml-2">
                          — {["Parcial", "Seguimiento", "Final"][e.tipoEvaluacion - 1] || "Tipo desconocido"} — Escala: <span className="font-medium">{e.escalaProgreso}/10</span>
                        </span>
                      </div>
                      <button
                ...
                {selected && (
                  <div className="mb-6">
                    <EvaluacionesPeriodicas
                      ...
                      pacienteId={selected.hcId?.toString()}
                      evaluacion={selected}
                    />
                  </div>
                )}
