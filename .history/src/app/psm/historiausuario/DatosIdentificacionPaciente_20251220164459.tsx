...
export default function DatosIdentificacionPaciente({
 ...
 const { getItem } = useInformacionPaciente();
  ...
  useEffect(() => {
    // Cambia el ID según tu lógica
    getItem(2)
      .then((data) => {
        if (data) {
          setPacienteExistente(data);
          setFormData(data); // Llena el formulario con los datos del paciente
        }
        setLoadingPaciente(false);
      })
      .catch(() => setLoadingPaciente(false));
  }, [getItem]);

  // Si está cargando, puedes mostrar un loader
  if (loadingPaciente) return <div>Cargando...</div>;

  // Determina si los campos deben estar deshabilitados
  const camposDeshabilitados = !!pacienteExistente;
