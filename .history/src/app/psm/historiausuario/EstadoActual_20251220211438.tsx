...
import type { EstadoMental } from "@/interfaces/historiaClinica/EstadoMental";

interface EstadoActualProps extends Partial<EstadoMental> {
  onChange?: (data: EstadoMental) => void;
  disabled?: boolean;
}
...
export default function EstadoActual({
  ...
  const [formData, setFormData] = useState<EstadoMental>({
    id: 0,
    hcId: 0,
  ...
  const handleInputChange = <K extends keyof EstadoMental>(
    field: K,
    value: EstadoMental[K]
  ) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
