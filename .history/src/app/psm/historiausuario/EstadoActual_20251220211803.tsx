...
export default function EstadoActual({
  ...
  // Usar el hook para obtener el registro
  const { data: estadoActual, isLoading } = useEstadoActual();
  // Si hay registro, el formulario está bloqueado; si no, está habilitado
  const isReadOnly = !!estadoActual;
  // Inicializar el estado del formulario según si hay registro o no
  ...
  // Si cambia el registro, actualizar el estado del formulario
  useEffect(() => {
    if (estadoActual) {
      setFormData(estadoActual);
    }
  }, [estadoActual]);
