...
export default function ImpresionDiagnostica({
  ...
  // Obtener el id de la historia clínica como número
  const hcId = typeof window !== "undefined"
    ? Number(localStorage.getItem("HistoriClinica") || 0)
    : 0;
  ...
  useEffect(() => {
    if (!hcId || hcId === 0) return setLoading(false);
    getItem(hcId)
      .then((data) => {
        if (data && data.id && data.id !== 0) {
          setImpresionExistente(data);
          setFormData({ ...data, hcId }); // Asegura que hcId esté correcto
        }
      })
  ...
  const handleInputChange = <K extends keyof ImpresionDiagnostica>(
    ...
    setFormData((prev) => ({
      ...prev,
      [field]: value,
      hcId, // Siempre actualiza hcId correctamente
    }));
  };
  ...
  const handleGuardar = async () => {
    try {
      if (!hcId || hcId === 0) throw new Error("No se encontró el id de la historia clínica");
      ...
      // Manejo específico para error 400 y mensaje del backend
