...
export default function ImpresionDiagnostica(props: ImpresionDiagnosticaProps) {
    ...
    const [existeDiagnostico, setExisteDiagnostico] = useState(false);
    const [loadingConsulta, setLoadingConsulta] = useState(true);

    // Simulación de consulta al servidor (reemplaza con tu lógica real)
    useEffect(() => {
      async function fetchDiagnostico() {
        setLoadingConsulta(true);
        // Aquí deberías consultar si ya existe un diagnóstico para el paciente/historia
        // Por ejemplo, usando un servicio o hook: const data = await getDiagnosticoByHcId(hcId);
        // Simulación:
        const data = null; // Cambia a un objeto para simular que sí existe
        setExisteDiagnostico(!!data);
