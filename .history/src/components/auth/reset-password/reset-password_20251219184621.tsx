...
const ResetPasswordChangeForm = ({ email }: { email: string }) => {
    ...
    const { changePassword, loading: loadingChange, error: errorChange } = useChangePassword();
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
                <button
                    className="flex items-center justify-center w-full px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600"
                    type="submit"
                    disabled={loadingVerify || loadingChange}
                >
                    Change Password
                </button>
            </div>
            {(errorVerify || errorChange) && (
                <div className="text-red-500">{errorVerify || errorChange}</div>
            )}
        </form>
    );
};
