...
const KanbanBoard = () => {
 ...
 return (
                    ...
                    {pendientes.map(cita => (
                      <div key={cita.id} className="p-5 bg-white border border-gray-200 task rounded-xl shadow-theme-sm dark:border-gray-800 dark:bg-white/5">
                        <div className="flex items-start justify-between gap-6">
                          <div>
                 <h4 className="text-lg font-semibold text-brand-500 dark:text-brand-400 flex items-center gap-2">
      <MdMedicalServices className="w-5 h-5" />
      {cita.servicioMedico.nombre}
    </h4>
    <span className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300">
  ...
  {cita.nota && (
    <div className="text-xs text-gray-500 dark:text-gray-400 italic border-l-4 border-brand-200 pl-2">
      {cita.nota}
    </div>
  )}
  <div className="flex items-center justify-between mt-2">
    <Link
      ...
      className="flex items-center gap-1 text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 font-medium transition"
