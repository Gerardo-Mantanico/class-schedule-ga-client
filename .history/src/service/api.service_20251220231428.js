...
const request = async (endpoint, method, data = null, customHeaders = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Si errorData es un objeto vacío, mostrar mensaje más claro
      const isEmptyObject = errorData && typeof errorData === 'object' && Object.keys(errorData).length === 0;
      console.error('Error API completo:', {
        status: response.status,
        statusText: response.statusText,
        endpoint: url,
        errorData: isEmptyObject ? 'Sin detalles de error en la respuesta' : errorData,
        fullError: message,
      });
