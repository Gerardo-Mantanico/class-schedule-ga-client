...
export default function SoportePaciente() {
  const [mensajes, setMensajes] = useState<Mensaje[]>([
    { id: 1, tcongreso: "Ejercicio de respiración", mensaje: "¿Cómo debo realizar la técnica correctamente?" },
    { id: 2, tcongreso: "Registro de emociones", mensaje: "¿Debo registrar cada emoción o solo las más importantes?" }
  ]);
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white dark:bg-gray-900 rounded-xl shadow-lg p-8">
      <h2 className="text-2xl font-bold mb-6 flex items-center gap-2 text-gray-900 dark:text-white">
        ...
        Soporte para tcongresos del paciente
      </h2>
      <ul className="divide-y divide-gray-200 dark:divide-gray-700">
        {mensajes.map(m => (
          <li key={m.id} className="py-6">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="font-semibold text-blue-700 dark:text-blue-300">{m.tcongreso}</span>
            </div>
        ...
        {mensajes.length === 0 && (
          <li className="text-gray-500 py-4 text-center">No hay tcongresos pendientes.</li>
        )}
      </ul>
    </div>
  );
