...
const TabWithLineAndIcon = () => {
 const [activeTab, setActiveTab] = useState<TabId>('Customers');
  ...
  const getTabClassName = (tabId : string) => {
    ...
    if (activeTab === tabId) {
      return `${baseClasses} text-brand-500 border-brand-500 dark:text-brand-400 dark:border-brand-400`;
    }
    
    return `${baseClasses} bg-transparent text-gray-500 border-transparent hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200`;
  };
...
export default TabWithLineAndIcon;